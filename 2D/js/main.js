(function(window,$,undefined){
	...
	$.extend({CA:function(o){
		...
		container,temp,ALL,	mat=[],next=[],
		t_step,t_loop,cnt_step=0,cnt_loop=0,
		isLock=false,isStep=false,isLock=false,isLoop=false,run_state=false;
		...
		function randnum(x,y){return parseInt(Math.random() * (x - y + 1) + y);};
		function getTemp(temp){var ret = {},thtml;for(t in temp)thtml = $(temp[t]).html(),ret[t]=thtml.substring(6,thtml.length-3);return ret;};
		function getM(x,y){return mat[x][y];};
		function getNode(x,y){return ALL.eq(y*op.size.x+x);};
		...
		function setCSS(obj,Css){for(i in Css)obj.css(i,Css[i]);};
...
(function(window,$,undefined){
	$(document).ready(function(){
		var t = $.CA();
		t.init({speed:10,size:{x:20,y:20},isClose:true,noHoldExt:false,noColor:false});
